...
class Path:
    ...
    def __init__(self, axes, speed, accel, cancelable=False,
                 use_bed_matrix=True, use_backlash_compensation=True,
    ...
    def get_arc_segments(self):
        ...
        start_t = self.inv_parametric_circle(start_point[0],
                                             start_point[0] + i,
                                             R)

        end_t = self.inv_parametric_circle(end_point[0],
                                           start_point[0] + i,
                                           R)
        ...
        X, Y = self.parametric_circle(arc_T, start_point[0] + i,
                                      start_point[1] + j,
                                      R)
        ...
        vals = np.transpose([np.linspace(
                            self.prev.ideal_end_pos[i],
                            self.ideal_end_pos[i],
                            num_segments) for i in xrange(self.printer.MAX_AXES)])
        ...
        for index, segment in enumerate(vec_segments):
            # print segment
            path = AbsolutePath(segment, self.speed, self.accel,
                                self.cancelable, self.use_bed_matrix, False)
            if index is not 0:
    ...
    def __str__(self):
        ...
        return ("Path from " + str(self.start_pos[:4]) +
                " to " + str(self.end_pos[:4]))
...
class RelativePath(Path):
    ...
    This is an approximate relative movement, i.e. we will move according to:
      (where we actually are) -> (somewhere close to = (where we think we
      are + our passed in vector)) but it should be pretty close!
    """
    def __init__(self, axes, speed, accel, cancelable=False,
                 use_bed_matrix=True, use_backlash_compensation=True,
                 enable_soft_endstops=True):
        Path.__init__(self, axes, speed, accel, cancelable,
                      use_bed_matrix, use_backlash_compensation,
...
class G92Path(Path):
    ...
    def set_prev(self, prev):
        ...
        if prev is not None:
            ...
            self.ideal_end_pos = np.zeros(self.printer.MAX_AXES,
                                          dtype=Path.DTYPE)
        ...
        # logging.debug(("G92 before: "+str(self.ideal_end_pos[:3])+
        #               " after: "+str(self.end_pos[:3])))
