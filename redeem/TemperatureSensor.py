...
class TemperatureSensor:
    ...
    def __init__(self, pin, heater_name, sensorIdentifier):
        ...
        # Find matching entry in sensor tables and instantiate
        ...
        if not found:
            for p in TemperatureSensorConfigs.pt100:
                if p[0] == self.sensorIdentifier:
                    wexp = "PT100 temperature sensor support is experimental."
                    logging.warning(wexp)
        ...
        try:
            with open(self.pin, "r") as file:
                ...
                if(signal > self.maxAdc or signal <= 0.0):
                    ...
                    # input range is 0 ... 1.8V
                    voltage = signal / self.maxAdc * 1.8
        except IOError as e:
            msg = "Unable to get ADC value ({0}): {1}".format(e.errno,
                                                              e.strerror)
            Alarm(Alarm.THERMISTOR_ERROR, msg)
...
""" This class represents standard thermistor sensors.
    It borrows heavily from Smoothieware's code:
    (https://github.com/Smoothieware/Smoothieware)
    wolfmanjm, thanks!
"""
...
class Thermistor(TemperatureSensor):
    ...
    def get_temperature(self, voltage):
        ...
        if r > 0:
            log = math.log(r)
            t = float((1.0 / (self.c1 + self.c2 * l + self.c3 * math.pow(log, 3))) - 273.15)
        else:
            t = -273.15
...
class PT100(TemperatureSensor):
    ...
    # The following calculations are based on the PT100 connected in the
    # same as as a thermistor.
    # Connecting it this way will give very low accuracy, but better accuracy
...
class Tboard (TemperatureSensor):
    ...
    def __init__(self, pin, sensorConfiguration, name):
        ...
        msg = "Initialized temperature sensor at {0} with temp/deg = {1}"
