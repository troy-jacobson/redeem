...
Handles the compilation of the PRU firmware based on the different
printer settings on the fly.
    ...
    def make_config_file(self):
        ...
        with open(configFile_0, 'w') as configFile:
            ...
            for name, stepper in self.printer.steppers.iteritems():
                ...
                confline = '#define STEPPER_{}_STEP_BANK\t\tSTEPPER_GPIO_{}\n'
                configFile.write(confline.format(name, step_bank))
                confline = '#define STEPPER_{}_STEP_PIN\t\t{}\n'
                configFile.write(confline.format(name, step_pin))
                confline = '#define STEPPER_{}_DIR_BANK\t\tSTEPPER_GPIO_{}\n'
                configFile.write(confline.format(name, dir_bank))
                confline = '#define STEPPER_{}_DIR_PIN\t\t{}\n'
            ...
            confline = '#define DIRECTION_MASK {}\n\n'.format(direction_mask)
            ...
            for name, endstop in self.printer.end_stops.iteritems():
                ...
                confline = '#define STEPPER_{}_END_PIN\t\t{}\n'.format(name,
                                                                       pin)
                configFile.write(confline)
                confline = '#define STEPPER_{}_END_BANK\t\tGPIO_{}_IN\n'
                configFile.write(confline.format(name, bank))
            ...
            for name, endstop in self.printer.end_stops.iteritems():
                ...
                for stepper in self.config.get('Endstops', option).split(","):
                    ...
                    if m is None:
                        msg = "'{}' is invalid for {}".format(stepper, option)
                        raise RuntimeError(msg)

                    # direction should be 1 for normal operation and -1 to
                    # invert the stepper.
                    if (m.group(2) == "pos"):
                        direction = -1
                    elif (m.group(2) == "neg"):
            ...
            # Put each dir and step pin in the proper buck if they are for
            # GPIO0 or GPIO1 bank. This is a restriction due to the limited
            # capabilities of the pasm preprocessor.
            for name, bank in banks.iteritems():
                # bank = (~bank & 0xFFFFFFFF)
                confline = "#define GPIO{}_MASK\t\t{}\n".format(name,
                                                                bin(bank))
                configFile.write(confline)
            ...
            confline = "#define END_STOP_DELAY {}\n".format(end_stop_delay)
            configFile.write(confline)
...
if __name__ == '__main__':
    ...
    from Stepper import (
        Stepper, Stepper_00A3, Stepper_00A4,
        Stepper_00B1, Stepper_00B2, Stepper_00B3
    )
    ...
    printer.steppers["X"] = Stepper("GPIO0_27", "GPIO1_29", "GPIO2_4",
                                    0, 0, "X")
    printer.steppers["Y"] = Stepper("GPIO1_12", "GPIO0_22", "GPIO2_5",
                                    1, 1, "Y")
    printer.steppers["Z"] = Stepper("GPIO0_23", "GPIO0_26", "GPIO0_15",
                                    2, 2, "Z")
    printer.steppers["E"] = Stepper("GPIO1_28", "GPIO1_15", "GPIO2_1",
                                    3, 3, "E")
    printer.steppers["H"] = Stepper("GPIO1_13", "GPIO1_14", "GPIO2_3",
                                    4, 4, "H")
    printer.steppers["A"] = Stepper("GPIO2_2", "GPIO1_18", "GPIO0_14",
                                    5, 5, "A")
    printer.steppers["B"] = Stepper("GPIO1_16", "GPIO0_5", "GPIO0_14",
                                    6, 6, "B")
    printer.steppers["C"] = Stepper("GPIO0_3", "GPIO3_19", "GPIO0_14",
                                    7, 7, "C")
