    ...
    def execute(self, g):
        if g.has_letter("P"):  # Load point
            ...
            try:
                ...
                logging.warning(
                    "G30 point P%d not yet defined. Aborting.", index)
                return
        else:
        ...
        bed_dist = round(self.printer.path_planner.probe(
            probe_length, probe_speed, probe_accel) * 1000.0, 3)  # convert to mm
        ...
        Alarm.action_command("bed_probe_point", json.dumps(
            [point["X"], point["Y"], bed_dist]))
...
class G30_1(GCodeCommand):
    ...
    def execute(self, g):
        ...
        self.printer.send_message(
            g.prot, "Offseting Z by {} to Z{}".format(bed_dist, Z_adjusted))
